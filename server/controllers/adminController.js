...
router.get('/wallpapers/:id', async (req, res) => {
    ...
    try {
        ...
        console.error('Error fetching wallpaper by ID:', error.message || error);
        res.status(500).json({ error: 'An error occurred while fetching the wallpaper' });
    }
});

router.get('/wallpapers/mongo/:mongoId', async (req, res) => {
    const { mongoId } = req.params;
    const { preview } = req.query;

    try {
        const fileContent = await adminServices.getWallpaperByMongoId(mongoId, preview === 'true');
        if (!fileContent) {
            return res.status(404).json({ error: 'File not found' });
        }
        ...
        res.set('Content-Type', 'image/jpeg');
        console.log('Sending file content:', fileContent.length);
        res.send(fileContent);
    } catch (error) {
